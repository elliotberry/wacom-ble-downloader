  ...
  async downloadAllNotes(outputDir) {
    ...
    try {
      ...
      for (let i = 0; i < fileCount; i++) {
        ...
        // Get file info (timestamp and byte count)
        const fileInfo = await this.protocol.getOldestFileInfo();
        console.log(`  File info: ${fileInfo.count} bytes, timestamp: ${fileInfo.timestamp ? new Date(fileInfo.timestamp * 1000).toISOString() : 'N/A'}`);
