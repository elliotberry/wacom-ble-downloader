...
class WacomBLE {
  ...
  logVerbose(levelOrMessage, message, ...args) {
    if (!this.verbose) {
      return;
    }
    let method = levelOrMessage;
    let text = message;
    if (typeof message === 'undefined') {
      method = 'detail';
      text = levelOrMessage;
    }
    const logFn = typeof logger[method] === 'function' ? logger[method] : logger.detail;
    logFn(text, ...args);
  }
  ...
  async scanAndConnect(timeout = 30000, registerMode = false) {
    return new Promise((resolve, reject) => {
      ...
      const scanSpinner = this.verbose ? null : logger.startSpinner('Scanning for Wacom devices');
      ...
      const stopScanningAndConnect = async () => {
        ...
        if (foundDevices.length === 0) {
          ...
          if (this.verbose) {
            logger.blank();
            logger.warn('No Wacom devices found.');
            logger.info('Troubleshooting tips:');
            logger.detail('1. Make sure your Wacom device is powered on');
            logger.detail('2. For registration: Hold the button for 6+ seconds until LED blinks');
            logger.detail('3. Make sure Bluetooth is enabled on your computer');
            logger.detail('4. Try moving closer to the device');
          }
      ...
      noble.on('discover', (peripheral) => {
        ...
        if (isWacom) {
          ...
          logger.success(`Found Wacom device: ${deviceInfo.name} ${status}${mode} (${displayAddress})`);
  ...
  async useSavedUUID(uuid) {
    ...
    logger.info('Authenticating with saved UUID...');
    try {
      ...
      if (result.success) {
        logger.success('Authenticated with saved UUID');
        return true;
      } else {
  ...
  handleFileTransferData(data) {
    ...
    if (this.verbose) {
      logger.detail(`Received ${buffer.length} bytes of file transfer data (total: ${this.fileTransferBuffer.length} bytes)`);
    }
  }
  ...
  async downloadAllNotes(outputDir, options = {}) {
    ...
    try {
      ...
      this.logVerbose('info', 'Preparing device for file transfer...');
      ...
      try {
        ...
        this.logVerbose('success', 'Device time synchronized');
      } catch (error) {
        this.logVerbose('note', `Could not set device time: ${error.message} (this is usually okay)`);
      }
      ...
      try {
        ...
        this.logVerbose('success', 'File transfer GATT configured');
      } catch (error) {
        this.logVerbose('note', `Could not configure transfer GATT: ${error.message} (this is usually okay)`);
      }
      ...
      this.logVerbose('info', 'Setting device to paper mode...');
      try {
        await this.protocol.setPaperMode();
        this.logVerbose('success', 'Device set to paper mode');
      } catch (error) {
      ...
      this.logVerbose('info', `Found ${fileCount} note(s) on device`);
      ...
      for (let i = 0; i < fileCount; i++) {
        this.logVerbose('info', `Downloading note ${i + 1}/${fileCount}...`);
        
        // Get file info (timestamp and stroke count)
        const fileInfo = await this.protocol.getOldestFileInfo();
        this.logVerbose('detail', `  File info: ${fileInfo.count} strokes, timestamp: ${fileInfo.timestamp ? new Date(fileInfo.timestamp * 1000).toISOString() : 'N/A'}`);
        
        // Download the file
        const strokeData = await this.protocol.getOldestFile();
        this.logVerbose('detail', `  Downloaded ${strokeData.length} bytes of raw data`);
        ...
        this.logVerbose('detail', `  Data hash (first 50 bytes sum): 0x${hash}`);
        ...
        this.logVerbose('detail', `  File count after deletion: ${newFileCount} (was ${fileCount - i})`);
        ...
        if (strokeData && strokeData.length > 0) {
          try {
            ...
            this.logVerbose('detail', `  First bytes: ${firstBytes}...`);
            ...
            this.logVerbose('detail', `  Parsed: ${drawing.strokes.length} stroke(s), ${totalPoints} point(s)`);
            if (firstPoint) {
              this.logVerbose('detail', `  First point: x=${firstPoint.x.toFixed(2)}, y=${firstPoint.y.toFixed(2)}, p=${firstPoint.p}`);
            }
            if (lastPoint) {
              this.logVerbose('detail', `  Last point: x=${lastPoint.x.toFixed(2)}, y=${lastPoint.y.toFixed(2)}, p=${lastPoint.p}`);
            }
            ...
            this.logVerbose('success', `Note ${i + 1} saved: ${filename} (${drawing.strokes.length} stroke(s), ${strokeData.length} bytes)`);
