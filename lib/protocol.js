...
class WacomProtocol {
  ...
  async getOldestFile() {
    ...
    if (startReply.length < 3 || startReply[2] !== 0xbe) {
      throw new Error(`Unexpected file transfer start response: ${Array.from(startReply).map(b => '0x' + b.toString(16)).join(' ')}`);
    }
    ...
    if (endReply.length < 3 || endReply[2] !== 0xed) {
      throw new Error(`Unexpected file transfer end response: ${Array.from(endReply).map(b => '0x' + b.toString(16)).join(' ')}`);
    }
    ...
    console.log(`Received ${fileData.length} bytes of file data (buffer had ${buffer.length} bytes)`);
    
    if (fileData.length === 0) {
      console.warn('Warning: No file data received!');
      console.warn(`Buffer state: length=${buffer.length}, complete=${this.transport.fileTransferComplete}`);
    } else if (fileData.length < 20) {
      console.warn(`Warning: File data seems too small (${fileData.length} bytes). First bytes: ${fileData.slice(0, Math.min(20, fileData.length)).map(b => '0x' + b.toString(16).padStart(2, '0')).join(' ')}`);
    }
  ...
  async deleteOldestFile() {
    ...
    try {
      ...
      if (reply.length < 3 || reply[2] !== 0x00) {
        ...
        if (errorCode === 0x1) {
          console.log('  Warning: Delete returned GENERAL_ERROR (0x1) - file may already be deleted');
          return; // Continue anyway
        } else if (errorCode === 0x2) {
          console.log('  Warning: Delete returned INVALID_STATE (0x2) - file may already be deleted');
          return; // Continue anyway
        }
      ...
      if (e.message.includes('timeout') || e.message.includes('No response')) {
        ...
        console.log('  Note: No reply to delete command (normal for Spark devices)');
      } else {
        throw e;
      }
    }
