...
class WacomProtocol {
  ...
  async getOldestFile() {
    ...
    if (fileData.length === 0) {
      ...
      logger.warn(`File data seems too small (${fileData.length} bytes). First bytes: ${fileData.slice(0, Math.min(20, fileData.length)).map(b => `0x${b.toString(16).padStart(2, '0')}`).join(' ')}`);
    }
