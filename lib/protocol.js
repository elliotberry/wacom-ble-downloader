...
class WacomProtocol {
  ...
  async getOldestFileInfo() {
    ...
    const msg = this.createMessage(0xc5, []);
    await this.transport.sendCommand(msg);
    ...
    try {
      ...
      count = reply1[2] | (reply1[3] << 8) | (reply1[4] << 16) | (reply1[5] << 24);
    } catch (e) {
      // Count reply may be missing
    }
    
    try {
      const reply2 = await this.transport.waitForReply(0xcd, 2000);
      // Parse timestamp: YYMMDDHHmmss format (6 bytes as hex)
      const timestampStr = reply2.slice(2).map(b => b.toString(16).padStart(2, '0')).join('');
      const year = 2000 + parseInt(timestampStr.substring(0, 2), 10);
      const month = parseInt(timestampStr.substring(2, 4), 10) - 1; // JS months are 0-indexed
      const day = parseInt(timestampStr.substring(4, 6), 10);
      const hour = parseInt(timestampStr.substring(6, 8), 10);
      const minute = parseInt(timestampStr.substring(8, 10), 10);
      const second = parseInt(timestampStr.substring(10, 12), 10);
  ...
  async getOldestFile() {
    ...
    const startMsg = this.createMessage(0xc3, []);
    await this.transport.sendCommand(startMsg);
    
    // Wait for file transfer start (0xc8 0xbe)
    const startReply = await this.transport.waitForReply(0xc8, 5000);
    if (startReply.length < 2 || startReply[1] !== 0xbe) {
      throw new Error('Unexpected file transfer start response');
    }
    
    // Wait for file transfer end (0xc8 0xed)
    ...
    if (endReply.length < 2 || endReply[1] !== 0xed) {
      throw new Error('Unexpected file transfer end response');
    }
    
    // Get the accumulated file data
    const buffer = this.transport.fileTransferBuffer;
  ...
  async deleteOldestFile() {
    const msg = this.createMessage(0xca, []);
    await this.transport.sendCommand(msg);
    // Delete doesn't require a reply
  }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (allowNoResponse) {
            console.log('Note: No response to CONNECT command (this may be normal for Slate devices)');
            return { success: true };
          }
          if (isRegistration) {
  ...
  async readTime() {
    ...
    try {
      ...
      console.log('Warning: Could not read device time (this is usually okay)');
      return null;
    }
  }
